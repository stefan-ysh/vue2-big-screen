  ...
  <div>
    <el-form label-width="80px" size="mini">
      ...
      <el-form-item v-if="attribute.event && attribute.eType === 'url'">
        <span slot="label">
          跳转链接
          <el-tooltip effect="dark" content="跳转至外部链接请填写完整路径，如：https://www.baidu.com" placement="left">
      ...
      <el-form-item v-if="attribute.event && attribute.eType === 'cpt'" class="event-wrap">
        ...
        <el-row v-for="(ev, index) in attribute.eventList" :key="index" :gutter="10">
          <el-col :span="11">
            <el-select v-model="ev.eMode" popper-class="big-screen-select">
          ...
          <el-col :span="11">
            <el-select
              v-model="ev.cpt"
              multiple=""
              :style="{ visibility: ev.eMode === 'refreshCanvas' ? 'hidden' : '' }"
