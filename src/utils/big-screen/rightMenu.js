import rightMenu from '../../components/components/RightMenu';
...
const install = function(Vue) {
  ...
  const rightCxtFn = function(options) {
    return new Promise((resolve, reject) => {
      ...
      // 处理X, Y坐标
      instance.posX =
        (options.event && options.event.clientX) || options.posX || 0
      instance.absoluteX = instance.posX
      instance.posY =
        (options.event && options.event.clientY) || options.posY || 0

      instance.nowBoxWidth = options.nowBoxWidth || 200 // 当前节点的宽度，不写则为200
      instance.needReverse =
        instance.posX + instance.nowBoxWidth > document.body.clientWidth - 5
